...
export default function FeatureBooks() {
    ...
    return (
        <section id="review" className="relative overflow-hidden w-full px-4 py-8 md:py-12">
            <motion.h2 className="text-center font-header text-2xl md:text-4xl italic text-accent font-bold mb-6 md:mb-8"
                       initial={{ opacity: 0 }}
            ...
            <div className="flex flex-col md:flex-row gap-6 w-full max-w-6xl mx-auto">
                <motion.div className="w-full md:w-1/3 order-1 md:order-2"
                ...
                <motion.div className="w-full md:w-2/3 order-2 md:order-1" initial={{ x:-100, opacity: 0 }}
                            whileInView={{ x:0,opacity: 1 }}
                            transition={{ duration: 1, type: "easeIn" }}
                            viewport={{ once: true, amount: 0.05 }}>
            ...
            <div className="mt-8 w-full max-w-6xl mx-auto grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-6 items-center justify-center align-middle justify-items-center">
                {loading
                    ?
                    <>
                        <div className="h-[100px] bg-gray-400 rounded  animate-pulse"></div>
                        <div className="h-[100px] bg-gray-400 rounded  animate-pulse"></div>
                        <div className="h-[100px] bg-gray-400 rounded  animate-pulse"></div>
                        <div className="h-[100px] bg-gray-400 rounded  animate-pulse"></div>
                        <div className="h-[100px] bg-gray-400 rounded  animate-pulse"></div>
                ...
                {
                    showBooks?.length === 0 &&
                    <motion.div className="max-w-sm w-full rounded-lg overflow-hidden shadow-lg bg-white border border-gray-200 hover:shadow-xl transition-shadow duration-300 m-2 relative flex flex-col sm:h-56 md:h-64 col-span-2 md:col-span-3"
                                initial={{opacity: 0, scale: 0}}
                                whileInView={{opacity: 1, scale: 1}}
                                transition={{duration: 1, type:"spring"}} viewport={{ once: true, amount: 0.05 }}>
                        <div className="p-4 sm:p-6 flex-grow flex flex-col justify-between">
                            <div>
                                ...
                                <div className="flex items-center my-3">
                                    ...
                                    <p className="text-gray-600 italic">We are not having Any Book</p>
                                </div>
                            </div>
                            ...
                            <div className="mt-4">
                                <div className="bg-blue-50 p-3 rounded-md border border-blue-100">
                                    <p className="text-sm text-blue-700">
                                        Try searching for a different category or check back later for new additions.
                                    </p>
                                </div>
                            </div>
                        </div>
            ...
            <div className="mt-4 flex justify-center items-center gap-3">
                {Array.from({ length: lengthPagination }).map((_, index) => (
                    <div
                        className={`cursor-pointer py-2 px-3 border border-accent border-solid text-sm rounded-xl text-semibold ${selectedPagination === index ? 'bg-highlight text-bg' : ''}`}
